package Com.jio.apiservice.Osprey.apiService.productSearch.ProductSearch.tasks;
...
public class ValidateProductSearchResponse extends BaseScript {
  ...
  //  String categoryL4 = "Citrus Fruits";
    ...
    public void validateResponse() {
        ...
        if (ospreyApiResponse.getDocs().get(0).getNameTextEn() == null) {
            softAssert.assertTrue(false, "Failed! Got null name");
        }
        if (ospreyApiResponse.getDocs().get(0).getPriceInrDouble()<=0) {
    ...
    public void validateTop12Results(String query) {
        ...
        String name = null;
        for (int i = 0; i < 12; i++) {
            name = ospreyApiResponse.getDocs().get(0).getNameTextEn();//get(i).getTitle();
            if (name.contains(query)== true) {
                log.info("Result " + i + " is::::" + ospreyApiResponse.getDocs().get(0).getNameTextEn());
            } else {
    ...
    public void validateSearchQueryResult(String query, String typeOfQuery) {
      //  if (productSearchResponse.getResults().get(0).getHits().get(0).getTitle().contains(query) == true)
        if(ospreyApiResponse.getDocs().get(0).getNameTextEn().contains(query) == true){
