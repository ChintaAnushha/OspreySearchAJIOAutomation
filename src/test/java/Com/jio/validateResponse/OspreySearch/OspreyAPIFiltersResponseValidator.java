...
//            }
    ...
    public void validateBrandFilterResponse() {
//        log.info("Starting brand filter and pagination validation");
//
...
//
//        // Get expected brand
//        String expectedBrand = testData.getOtherParams().get("values");
...
//
//        // Get brand facet data and count
...
//        int facetCount = getBrandFacetCount(facetData, expectedBrand);
//
////        List<Map<String, Object>> brandFacets = facetData.get("brand_string_mv");
////        if (brandFacets != null) {
////            log.info("Processing brand facets, size: " + brandFacets.size());
////            for (Map<String, Object> facet : brandFacets) {
////                String brandName = (String) facet.get("name");
////                log.info("Checking brand facet: " + brandName + " against expected: " + expectedBrand);
////
////                if (brandName.toLowerCase().contains(expectedBrand.toLowerCase())) {
////                    Object valueObj = facet.get("value");
////                    if (valueObj instanceof Number) {
////                        facetCount = ((Number) valueObj).intValue();
////                        log.info("Found brand facet count: " + facetCount);
////                        break;
////                    } else if (valueObj != null) {
////                        try {
////                            facetCount = Integer.parseInt(String.valueOf(valueObj));
////                            //    expectedBrandCount = Integer.parseInt(valueObj.toString());
////                            log.info("Found matching brand facet count (parsed): " + facetCount);
////                            break;
...
////            }
////        }
//////        log.info("\n========== Brand Facet Analysis ==========");
//////        log.info("Expected Brand: " + expectedBrand);
//////     //   log.info("Found Facet Count: " + expectedBrandCount);
////
//        // Analyze products and count matches
//        int brandMatchCount = 0;
//        StringBuilder productDetails = new StringBuilder();
//        log.info("\n========== Product Brand Analysis ==========");
//
//        for (OspreyApiResponse.Doc product : products) {
//            List<String> productBrands = product.getBrandNameTextEnMv();
//            String productName = product.getNameTextEn();
//
//            boolean matches = false;
//            if (productBrands != null) {
//                String normalizedExpectedBrand = expectedBrand.toLowerCase().trim();
//                log.debug("Checking product: " + productName);
//
//                // Print brand comparison for debugging
//                log.debug("Checking product: " + productName);
//                if (productBrands != null) {
//                    productBrands.forEach(brand ->
//                            log.debug("Comparing brand: '" + brand + "' with expected: '" + normalizedExpectedBrand + "'")
//                    );
//                }
//
//                matches = productBrands.stream()
//                        .filter(Objects::nonNull)
//                        .map(String::toLowerCase)
//                        .map(String::trim)
//                        .anyMatch(brand -> {
//                            boolean isMatch = brand.equals(normalizedExpectedBrand);
//                            if (isMatch) {
//                                log.debug("Found match: " + brand);
//                            }
//                            return isMatch;
//                        });
//            }
////                String finalExpectedBrand = expectedBrand.toLowerCase();
////                matches = productBrands.stream()
////                        .filter(Objects::nonNull)
////                        .map(brand -> brand.toLowerCase().trim())
////                        .anyMatch(brand -> brand.equals(finalExpectedBrand));
//           // }
////                for (String brand : productBrands) {
////                    if (brand != null)// && brand.toLowerCase().equals(expectedBrand)) {
////                        {
////                            String normalizedBrand = brand.toLowerCase().trim();
////                            String normalizedExpected = expectedBrand.toLowerCase().trim();
////                            if (normalizedBrand.equals(normalizedExpected)) {
////                                matches = true;
////                        break;
////                    }
////                }
...
////      }
////       String finalExpectedBrand = expectedBrand.toLowerCase();
////            boolean matches = productBrands != null && productBrands.stream()
////                    .map(String::toLowerCase)
////                    .anyMatch(brand -> brand.contains(finalExpectedBrand));
//
//            if (matches) {
//                brandMatchCount++;
//                log.debug("Found matching brand for product: " + productName);
//            } else {
//                log.debug("No brand match for product: " + productName + ", brands: " + productBrands);
//            }
//
//            String productLog = String.format(
//                    "Product Details:\n" +
//                            "Name: %s\n" +
//                            "Brand(s): %s\n" +
//                            "Matches Expected Brand '%s': %s\n" +
//                            "------------------------",
//                    productName,
...
//            );
//            log.info(productLog);
//            productDetails.append(productLog).append("\n");
//
//            // Add debug logging before the validation
//            log.info("\n========== Brand Match Summary ==========");
//            log.info("Expected Brand (normalized): '" + expectedBrand.toLowerCase().trim() + "'");
//            log.info("Total Products: " + docsCount);
//            log.info("Matching Products Found: " + brandMatchCount);
////
////            if (matches) {
////                brandMatchCount++;
////            }
...
//            log.info(String.format("Total Count: %d", numFoundCount));
...
//            log.info(String.format("All Products Match Filter: %s", docsMatchFilter ? "Yes" : "No"));
//            log.info(String.format("Facet Matches NumFound: %s", facetMatchesNumFound ? "Yes" : "No"));
//            log.info(String.format("Docs Match NumFound: %s", docsMatchNumFound ? "Yes" : "No"));
//            log.info(String.format("Overall Status: %s", allCountsMatch ? "PASSED" : "FAILED"));
//
//            // Force test failure if counts don't match
//            if (!allCountsMatch) {
...
//                                "Docs Count: %d\n" +
//                                "Matching Products: %d",
//                        numFoundCount, facetCount, docsCount, brandMatchCount
//                );
//                log.error(errorMessage);
//                Assert.fail(errorMessage);
//            }

        log.info("Starting brand filter validation");
        ...
        // Get facet count
        Map<String, List<Map<String, Object>>> facetData = ospreyApiResponse.getFacetData();
        int facetCount = numFoundCount; // Default to numFound
